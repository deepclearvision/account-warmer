...
async def load_accounts() -> list[dict]:
    """Return desktop accounts via AccountStore."""
    from core.account_store import get_account_store
    return get_account_store().get_desktop_accounts()


async def save_accounts(accounts: list[dict]) -> None:
    """Save desktop accounts via AccountStore."""
    from core.account_store import get_account_store
    async with _accounts_lock:
        get_account_store().save_desktop_accounts(accounts)


async def load_mobile_accounts() -> list[dict]:
    """Return mobile accounts via AccountStore."""
    from core.account_store import get_account_store
    return get_account_store().get_mobile_accounts()


async def save_mobile_accounts(accounts: list[dict]) -> None:
