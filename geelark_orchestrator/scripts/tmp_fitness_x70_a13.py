...
PROFILE_KEY = "gracelynvillanueva9987"
MODEL = "X70"
...
# Verify mock persists at +0s, +5s, +15s, +30s
for delay in [0, 5, 15, 30]:
    if delay > 0:
        time.sleep(delay)
    mock_ok, mock_info = dumpsys_mock_check(PHONE_ID)
    print(f"    Mock persistence (+{delay}s): {'PASS' if mock_ok else 'FAIL'} | {mock_info}")
    if not mock_ok:
...
has_search_term = 'plumber' in xml
has_clear_btn = 'Clear' in xml
