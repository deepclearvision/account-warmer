#!/usr/bin/env python3
"""
Unified warmup runner. One script, no copy-rebuild bugs.

Usage:
  python _run.py brand-1km acc_004
  python _run.py money-kw acc_004
  python _run.py local-discovery acc_004
  python _run.py brand-1km acc_004 leave
  python _run.py money-kw acc_004 "custom keyword"
"""
...
# ── Parse args ──────────────────────────────────────────────────────────────
if len(sys.argv) < 3:
    print("Usage: python _run.py <mode> <acc_id> [keyword|leave] [leave]")
...
# Both _run.py and mobile_warmup.py check this file before starting a phone
...
try:
    ...
    # Verify IP is different from last run
    if current_ip:
        last_ip = None
        if LOCK_FILE.exists():
            try:
        ...
        # Store current IP in lock for next run's comparison
    ...
    if MODE in ("brand-1km", "money-kw"):
        ...
        # ── Open a listing ───────────────────────────────────────────────────
        # Try to find a listing label (Open, Closed, distance, etc.) and tap it.
        # Fall back to a coordinate tap in the results area if no label is found.
        xml = dump_ui("ld_list")
        ...
        # Try to find and tap a business-detail button (Directions, Call,
        # Website, Photos, Reviews, Save, Share).  If one is found, dwell and
        # scroll the detail page.  Otherwise still spend time reading.
        ...
        if detail:
            ...
            # Swipe back to top of listing
            sh("input swipe %d %d %d %d 500" % (w//2, int(h*0.40), w//2, int(h*0.80)))
            time.sleep(2)
            # If Website opened Chrome, one Back returns to Maps listing;
            # if still on Maps, Back returns to search results.
            sh("input keyevent KEYCODE_BACK"); time.sleep(2)
            focus = sh("dumpsys window | grep mCurrentFocus").strip()
            if "maps" not in focus.lower() and "chrome" not in focus.lower():
                sh("input keyevent KEYCODE_BACK"); time.sleep(1)
        else:
            # On a listing but didn't tap a specific button — still browse
    ...
    if 'PHONE' in dir() and PHONE:
        if not phone_started:
            pass  # Never got to phone start, nothing to stop
        elif LEAVE_OPEN:
...
print("Keyword: %s | Total time: %.0fs (%.1f min)" % (KEYWORD, total, total/60))
if MODE in ("brand-1km", "local-discovery"):
