"""Test full search chain with coord click and notification-aware handling."""
import json
import sys
...
fb = FlowBuilder(
    title="Coord search chain",
    desc="Full chain with coord click",
    timeout_minutes=5,
    error_type="skip",
)
fb.open_app(MAPS_PACKAGE, remark="Open Maps")
fb.wait(5000, "Wait for Maps")
# Coord click on search box (avoids Play Services notification interception)
fb.click_coord(540, 160, remark="Tap search box")
...
# Try clicking business name at top
fb.click("text", BUSINESS_NAME, search_time_ms=8000, remark="Try click business (top)")
...
if data:
    ...
    path = p / "coord_search_chain_mid.png"
...
data = client.take_screenshot(PHONE_ID, max_wait=30)
if data:
    path = p / "coord_search_chain_final.png"
    path.write_bytes(data)
