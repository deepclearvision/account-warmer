"""Test full Maps search chain on acc_049."""
import json
import sys
...
fb = FlowBuilder(
    title="Test Maps chain",
    desc="Open Maps, search, submit, find business",
    timeout_minutes=10,
    error_type="skip",
)
...
# Take 3 screenshots during execution
...
if data:
    ...
    path = p / "test_maps_chain_mid.png"
...
data = client.take_screenshot(PHONE_ID, max_wait=30)
if data:
    path = p / "test_maps_chain_late.png"
    path.write_bytes(data)
...
data = client.take_screenshot(PHONE_ID, max_wait=30)
if data:
    path = p / "test_maps_chain_final.png"
    path.write_bytes(data)
