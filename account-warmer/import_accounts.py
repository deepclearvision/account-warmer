...
from core.paths import ACCOUNTS_FILE as CONFIG_FILE, GEELARK_ACCOUNTS_FILE
...
def load_geelark_accounts() -> dict:
    if GEELARK_ACCOUNTS_FILE.exists():
        with open(GEELARK_ACCOUNTS_FILE, encoding="utf-8") as f:
            data = yaml.safe_load(f) or {}
        if "accounts" not in data:
            data["accounts"] = []
        return data
...
def save_geelark_accounts(data: dict) -> None:
    ...
    with open(GEELARK_ACCOUNTS_FILE, "w", encoding="utf-8") as f:
        yaml.dump(data, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
        ...
        for row_num, row in enumerate(reader, start=2):
            ...
            if strategy_raw:
                if strategy_raw not in VALID_STRATEGIES:
                    errors.append(f"Row {row_num}: unknown strategy {strategy_raw!r} — skipped")
                else:
