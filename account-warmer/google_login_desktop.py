...
including TOTP 2FA. Saves result to WarmingData/logs/state/login_status.json.
...
def _load_credentials(acc_id: str) -> dict:
    """Pull password + totp_secret from geelark_accounts.yaml."""
    ...
    acc = next((a for a in accounts if a.get("id") == acc_id), {})
    return {
...
async def login_account(account: dict) -> bool:
    ...
    try:
        ...
        data = {}
    data[acc_id] = {
        "status":     "logged_in" if success else "login_failed",
