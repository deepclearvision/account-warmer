...
from core.paths import (
    SCHEDULE_FILE, STRATEGIES_FILE, GOALS_FILE, BEHAVIOUR_FILE,
    BUSINESSES_FILE, STATE_DIR, ACCOUNTS_FILE, LOGS_DIR,
    GEELARK_ACCOUNTS_FILE,
)
...
# Activities that must never be run automatically — user performs them manually.
_MANUAL_ONLY_ACTIVITIES = {
    "maps_review",      # writing reviews
    "review_write",
    "pindrop",          # pin drops
    "pin_drop",
    "phone_edit",       # phone number edits
    "gmb_create",       # Google Business Profile creation
    "gmb_apply",
}


def _replace_goal_actions(activities: list) -> list:
    """Replace any manual-only activities with maps_browse (safe passive substitute)."""
    return [
        "maps_browse" if a in _MANUAL_ONLY_ACTIVITIES else a
        for a in activities
    ]
...
class AccountOrchestrator:
    ...
    def __init__(self, account: dict, all_accounts: list, dry_run: bool = False,
        ...
        # Goals take precedence over strategies
        goal = account.get("goal")
        if goal and GOALS_FILE.exists():
            all_goals = _load_yaml(GOALS_FILE)
            resolved = all_goals.get(goal)
            if resolved is None:
                self.log.warning(f"Unknown goal {goal!r} — falling back to standard schedule")
    ...
    async def run_session(self) -> None:
        ...
        # Maybe skip today entirely
        skip_prob = week_cfg.get("skip_day_probability", 0.10)
        if random.random() < skip_prob and self.force_week is None:
        ...
        if weeks_elapsed >= 1:
            ...
            if biz_activity and "business_signal" not in activities:
        ...
        # ── Goal-action gate ────────────────────────────────────────────────────
        # When a goal account reaches its goal_action_week, flag it for MANUAL
        # action rather than running any goal-action activities automatically.
        # Reviews, pin drops, phone edits, and GBP creation must all be done
        # by the user in the browser — the warmer never performs them.
        goal_action_week = self.schedule.get("goal_action_week")
        if goal_action_week and (weeks_elapsed + 1) >= goal_action_week:
            activities = _replace_goal_actions(activities)
            self._flag_manual_action_ready()
    ...
    async def _run_activity(self, name: str, page, emails_sent_today: int) -> bool:
        ...
        try:
            if name == "search":
                ...
                return await MapsActivity(page, self.account, self.behaviour, browse_only=True).run()
            elif name == "maps_review":
                from activities.maps import MapsActivity
                return await MapsActivity(page, self.account, self.behaviour, browse_only=False).run()
    ...
    def _get_pending_business_signal(self) -> str | None:
        ...
        target_ids = self.account.get("target_businesses", [])
        if not target_ids:
            return None
    ...
    def _get_mobile_done_today(self) -> set[str]:
        """
        Return a set of desktop activity names that were already performed today
        on this account's paired GeelarK mobile phone.

        Looks up the matching gl_xxx entry in geelark_accounts.yaml by email,
        then reads today's entries from logs/mobile_sessions.json.
        """
        account_email = self.account.get("email", "").lower()
        if not account_email:
            return set()

        # Find the matching GeelarK account id by email
        gl_id: str | None = None
        try:
            if GEELARK_ACCOUNTS_FILE.exists():
                gl_data = _load_yaml(GEELARK_ACCOUNTS_FILE)
                for gl_acc in gl_data.get("accounts", []):
                    if gl_acc.get("email", "").lower() == account_email:
                        gl_id = gl_acc.get("id")
                        break
        except Exception as e:
            self.log.debug(f"[mobile-coord] Could not read geelark_accounts.yaml: {e}")
            return set()
        ...
        try:
            ...
            self.log.debug(f"[mobile-coord] Could not read mobile_sessions.json: {e}")
            return set()
    ...
    def _choose_activities(self, week_cfg: dict) -> list[str]:
        ...
        focus_pools = {
            ...
            "maps_focus":    ["maps_browse", "maps_review"],
    ...
    def _flag_manual_action_ready(self) -> None:
        """
        Mark this account as ready for a manual goal action.
        Sets manual_action_ready=true and manual_action_type=<goal> in accounts.yaml
        if not already set.  This is a one-time write — cleared when the user marks
        the action as done via the dashboard.
        """
        if self.account.get("manual_action_ready"):
            return  # already flagged
        goal = self.account.get("goal", "unknown")
        _GOAL_LABELS = {
            "reviews_uk":    "Write a Review",
            "pindrop_uk":    "Drop a Pin",
            "phone_edit_uk": "Edit Phone Number",
            "gmb_creation":  "Create GBP Listing",
        }
        action_label = _GOAL_LABELS.get(goal, goal.replace("_", " ").title())
        try:
            ...
            for acc in accounts:
                if acc["id"] == self.account["id"]:
                    acc["manual_action_ready"] = True
                    acc["manual_action_type"]  = goal
                    acc["manual_action_label"] = action_label
                    acc["manual_action_since"] = str(date.today())
                    break
            ...
            self.log.info(
                f"[orchestrator] Account ready for manual action: {action_label} — "
                f"open the Multilogin browser and perform this action manually, "
                f"then click 'Mark Done' in the dashboard."
            )
        except Exception as e:
            self.log.warning(f"[orchestrator] Could not flag manual_action_ready: {e}")
