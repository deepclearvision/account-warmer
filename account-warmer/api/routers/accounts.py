...
from core.paths import GOALS_FILE
VALID_GOALS = ["reviews_uk", "pindrop_uk", "phone_edit_uk", "gmb_creation"]
_GOAL_LABELS = {
    "reviews_uk":    "Reviews for UK Businesses",
    "pindrop_uk":    "Pindrop for UK Businesses",
    "phone_edit_uk": "Phone Number Edit for UK Businesses",
    "gmb_creation":  "Warming for GMB Creation",
}
...
def _phase_label(strategy: str, weeks_elapsed: int, strategies: dict,
                 goal: str | None = None, goals: dict | None = None) -> str:
    band = _week_band(weeks_elapsed)
    if goal and goals:
        goal_cfg = goals.get(goal, {})
        lbl = goal_cfg.get(band, {}).get("label")
        if lbl:
            return lbl
...
def _goal_emulator_note(goal: str | None, goals: dict | None,
                        weeks_elapsed: int) -> str | None:
    if not goal or not goals:
        return None
    cfg = goals.get(goal, {})
    em = cfg.get("emulator", {})
    add_at = em.get("add_at_week", 99)
    duration = em.get("duration_weeks", 0)
    if weeks_elapsed < add_at:
        return f"Add to emulator at week {add_at}"
    if weeks_elapsed < add_at + duration:
        return em.get("note", "Emulator active")
    return None
...
def _enrich(account: dict, strategies: dict, cache: dict | None = None,
            trust_checks: dict | None = None, goals: dict | None = None) -> dict:
    ...
    goal_cfg  = (goals or {}).get(goal, {}) if goal else {}
    goal_meta = {
        "goal":               goal,
        "goal_display_name":  _GOAL_LABELS.get(goal) if goal else None,
        "goal_estimated_weeks": goal_cfg.get("estimated_weeks"),
        "goal_action_week":   goal_cfg.get("goal_action_week"),
        "goal_engagement_week": goal_cfg.get("engagement_start_week"),
        "emulator_note":      _goal_emulator_note(goal, goals, weeks_elapsed),
    }
...
def _load_goals() -> dict:
    try:
        return load_yaml_sync(GOALS_FILE) if GOALS_FILE.exists() else {}
    except Exception:
        return {}
...
async def list_accounts():
    ...
    return [_enrich(a, strategies, cache, trust, goals) for a in accounts]
...
@router.get("/goals")
def list_goals():
    """Return full goal configurations including per-phase detail for the planner UI."""
    goals_data = _load_goals()
    result = []
    for gid in VALID_GOALS:
        cfg = goals_data.get(gid, {})
        phases = []
        for band_key, band_label in _PHASE_BANDS:
            band = cfg.get(band_key, {})
            if not band:
                continue
            weights = band.get("activity_weights", {})
            total_w = sum(weights.values()) or 1
            activities = [
                {
                    "id":      act,
                    "label":   _ACTIVITY_LABELS.get(act, act),
                    "weight":  round(w / total_w * 100),
                }
                for act, w in sorted(weights.items(), key=lambda x: -x[1])
            ]
            phases.append({
                "band":               band_key,
                "band_label":         band_label,
                "label":              band.get("label", band_label),
                "phase_note":         band.get("phase_note", ""),
                "actions_per_day":    band.get("actions_per_day", [1, 3]),
                "skip_probability":   band.get("skip_day_probability", 0.10),
                "searches_per_session": band.get("searches_per_session", [1, 3]),
                "session_gap_hours":  band.get("session_gap_hours", [4, 12]),
                "activities":         activities,
            })
        em = cfg.get("emulator", {})
        result.append({
            "id":                  gid,
            "display_name":        _GOAL_LABELS.get(gid, gid),
            "description":         cfg.get("description", ""),
            "estimated_weeks":     cfg.get("estimated_weeks"),
            "engagement_start_week": cfg.get("engagement_start_week"),
            "goal_action_week":    cfg.get("goal_action_week"),
            "emulator": {
                "add_at_week":      em.get("add_at_week"),
                "duration_weeks":   em.get("duration_weeks"),
                "searches_per_day": em.get("searches_per_day"),
                "note":             em.get("note", ""),
            },
            "phases": phases,
        })
...
async def update_account(account_id: str, body: AccountUpdate):
    ...
    if body.goal is not None:
        if body.goal and body.goal not in VALID_GOALS:
            raise HTTPException(400, f"Invalid goal: {body.goal!r}. "
                                     f"Valid: {VALID_GOALS}")
        if body.goal:
    ...
    from api.deps import load_trust_checks
    trust = load_trust_checks()
    return _enrich(target, strategies, cache, trust, goals)


@router.delete("/{account_id}")
async def delete_account(account_id: str):
    """Remove an account from accounts.yaml. Does NOT delete the Multilogin profile."""
    accounts = await load_accounts()
...
async def bulk_delete(body: dict):
    """Delete multiple accounts from accounts.yaml by ID list."""
    ids_to_delete = set(body.get("ids", []))
    if not ids_to_delete:
        raise HTTPException(400, "No account IDs provided")
    accounts = await load_accounts()
    new_list = [a for a in accounts if a["id"] not in ids_to_delete]
    removed  = len(accounts) - len(new_list)
    await save_accounts(new_list)
    return {"deleted": removed, "ids": list(ids_to_delete)}
...
async def bulk_update(body: dict):
    ...
    allowed = {"goal", "strategy", "active_hours", "warmup_start_date", "target_businesses", "tags", "category", "geo_city"}
    updates = {k: v for k, v in updates.items() if k in allowed}
    if not ids or not updates:
        raise HTTPException(400, "Provide ids and at least one allowed field to update")
    if "goal" in updates and updates["goal"] and updates["goal"] not in VALID_GOALS:
        raise HTTPException(400, f"Invalid goal: {updates['goal']!r}")
...
async def import_from_csv(body: CsvImportRequest):
    ...
    Required columns : email, password, totp_secret
    Optional columns : location, proxy, geo_area, strategy, category

    Creates entries in both accounts.yaml (for ML warmer) and
    geelark_accounts.yaml (for mobile warmer).  Optionally provisions
    a GeelarK cloud phone for each new account.
    """
    import csv
    import io
    import os
    ...
    gl_data     = yaml.safe_load(gl_file.read_text(encoding="utf-8")) if gl_file.exists() else {}
    gl_accounts = (gl_data or {}).get("accounts", [])
    ...
    UPDATABLE = {
        "category", "location", "geo_city", "geo_area",
    ...
    for row_num, row in rows:
        ...
        acc_entry = {
            "id":                    f"acc_{n_acc:03d}",
            "email":                 email,
            "multilogin_profile_id": "",   # to be linked via ML sync later
        ...
        for field in ("category", "geo_area", "geo_city", "home_address"):
            if row.get(field):
    ...
    gl_file.write_text(
        yaml.dump({"accounts": gl_accounts}, default_flow_style=False,
...
async def csv_template():
    """Return a sample CSV template for account import."""
    from fastapi.responses import PlainTextResponse
    template = (
        "email,password,totp_secret,location,proxy,geo_area,strategy,category\n"
        "john.smith1985@gmail.com,MyPassword123!,BASE32TOTPSECRETHERE,London UK,,greenwich,maps_heavy,batch_1\n"
        "jane.doe1990@gmail.com,AnotherPass456!,ANOTHERBASE32SECRET,London UK,,clapham,maps_heavy,batch_1\n"
    )
...
async def import_from_ml(profiles: list[dict]):
    ...
    # Load geelark_accounts.yaml to avoid duplicate phone creation
    gl_file = DATA_DIR / "geelark_accounts.yaml"
    if gl_file.exists():
        gl_data = yaml.safe_load(gl_file.read_text(encoding="utf-8")) or {}
        gl_accounts = gl_data.get("accounts", [])
    else:
        gl_accounts = []
    gl_emails = {a.get("email", "").lower() for a in gl_accounts}
    gl_max_n  = max(
...
@router.post("/{account_id}/mark-action-done")
async def mark_manual_action_done(account_id: str):
    """
    Clear the manual_action_ready flag after the user has performed the goal action
    manually in the browser.  Also records the date it was completed.
    """
    ...
    for acc in accounts:
        if acc["id"] == account_id:
            acc["manual_action_ready"] = False
            acc["manual_action_done_at"] = str(_date.today())
            found = True
            break
    if not found:
        raise HTTPException(404, f"Account {account_id!r} not found")
