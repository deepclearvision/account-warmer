...
class CalendarActivity(BaseActivity):
    ...
    async def _create_event(
        ...
        try:
            ...
            # --- Save ---
            await self.humaniser.pause(2000, 5000)
            # Google Calendar uses both <button> and <div role="button"> for Save
            save_btn = self.page.locator(
                'button[aria-label="Save"], button:has-text("Save"), '
                'div[role="button"][aria-label="Save"], '
                'div[role="button"]:has-text("Save"), '
                '[jsname] button:has-text("Save"), '
                'div[jscontroller] div[role="button"]:has-text("Save")'
            ).first
            if await save_btn.is_visible(timeout=5000):
                ...
                self.log.debug("Save button not found — trying Ctrl+S")
                await self.page.keyboard.press("Control+s")
                await self.humaniser.pause(2000, 4000)
                # Check if the event editor closed (meaning it saved)
                title_check = self.page.locator(
                    'input[data-eventchip-title], input[placeholder*="title" i]'
                ).first
                if not await title_check.is_visible(timeout=3000):
                    self.log.info(f"Event created via shortcut: {title!r} on {event_date}")
                    return True
    ...
    async def _dismiss_consent(self) -> None:
        """Dismiss Google Calendar consent dialogs."""
