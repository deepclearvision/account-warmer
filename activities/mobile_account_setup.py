...
def _install_gps_app(phone_id: str, acc_id: str) -> bool:
    """
    Install the mock GPS spoofing app (e.g. Lexa Fake GPS) via GeelarK upload.
    This is critical: without it, Maps and Timeline will not see our spoofed
    coordinates and the account's location history will be empty.
    """
    from core.gps_spoofing import ensure_installed
    log.info("[%s] Ensuring mock GPS app is installed …", acc_id)
    ok = ensure_installed(phone_id)
    log.info("[%s] Mock GPS app: %s", acc_id, "OK" if ok else "FAILED (see logs)")
    return ok
...
def run_account_setup(account: dict) -> dict:
    ...
    steps = [
        ...
        ("install_gps_app",         lambda: _install_gps_app(phone_id, acc_id)),
