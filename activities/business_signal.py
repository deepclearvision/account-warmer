...
business. Signals are split across three phases spread over multiple sessions:
  ...
  Phase 3 — Post-visit (after appointment date has passed):
    - Search for the business again (post-visit behaviour)
    - View the Maps listing briefly
    - Log completion

Each phase only runs once per business per account — state is tracked.
Phases advance automatically as sessions run.

Reviews are NOT triggered through this module — desktop warming is
account warming only.  Business actions happen elsewhere.
"""
...
class BusinessSignalActivity(BaseActivity):
    ...
    async def _view_maps_listing(self) -> None:
        ...
        search_query = f"{self._biz['name']} {self._biz.get('address', '')}".strip()

        await self.page.goto(MAPS_URL, wait_until="domcontentloaded")
        await self.humaniser.pause(2000, 4000)
        await self._dismiss_consent_maps()

        # Try the search box; fall back to direct URL navigation if unavailable
        search_box = await self._find_maps_search_box()
        if search_box:
            ...
            # Fallback: navigate directly to the Maps search URL
            self.log.debug("Search box not found — using direct URL navigation")
        ...
        # Verify and click the target business listing
        if not await self._click_business_result():
            self.log.warning(
                f"Could not verify business {self._biz['name']!r} — "
                f"skipping listing interaction"
            )
            return
    ...
    async def _get_directions_to_business(self) -> None:
        ...
        try:
            ...
            if listing_url:
                ...
                if not await self._click_business_result():
                    self.log.debug(
                        "Directions: business not verified from saved URL — "
                        "falling back to address search"
                    )
                else:
            ...
            # Fallback: type address in search box, or use direct URL
            import urllib.parse
            search_box = await self._find_maps_search_box()
            if search_box:
                await search_box.click()
    ...
    async def _view_maps_listing_briefly(self) -> None:
        ...
        try:
            await self.page.goto(listing_url, wait_until="domcontentloaded")
            await self.humaniser.pause(2000, 5000)
            if await self._click_business_result():
    ...
    async def _run_phase_3(self) -> None:
        ...
        self._log_interaction(3, "Post-visit complete", shot)

        self._state["phase_3_complete"] = True
        self._state["phase_3_date"]     = str(date.today())
        self.log.info(f"Phase 3 complete: {self._biz['name']!r}")
    ...
    async def _click_business_result(self) -> bool:
        """
        Verify and click the Maps result that matches our target business.
        Returns True if the business was found and clicked, False otherwise.
        """
        result = await self.verify_business_result(
            expected_name=self._biz["name"],
            expected_address=self._biz.get("address"),
        )
        if result:
            ...
            self.log.warning(
                f"Business {self._biz['name']!r} NOT found in Maps results"
            )
            return False
    ...
    async def _ensure_on_listing(self) -> None:
        ...
        if listing_url and "maps.google" not in self.page.url and "/maps/" not in self.page.url:
            ...
            await self.humaniser.pause(2000, 4000)
            if not await self._click_business_result():
                self.log.debug("_ensure_on_listing: business not found on re-navigation")
    ...
    async def _dismiss_cookie_banner(self) -> None:
        """Dismiss third-party cookie banners."""
        await self.dismiss_cookie_banner()

    async def _find_maps_search_box(self):
        """Wait for the Maps search box and return its locator, or None if unavailable."""
