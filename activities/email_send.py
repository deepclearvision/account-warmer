...
class EmailSendActivity(BaseActivity):
    ...
    async def _send_email(self, to_address: str) -> bool:
        try:
            ...
            # Dismiss any Gmail first-time onboarding popups
            # (Welcome, Choose view, Smart features, Chat, Profile photo, etc.)
            await self.dismiss_gmail_onboarding()
            ...
            # Fill To field — Gmail uses a contenteditable div, not <input>
            # pressSequentially works on any focusable element (div, input, textarea)
            to_field = self.page.locator(
                'input[name="to"], textarea[name="to"], '
                '[aria-label="To"] div, [aria-label="To"][contenteditable], '
                'div[name="to"]'
            ).first
            await to_field.click()
            await self.humaniser.pause(300, 700)
            # Clear any existing text, then type the recipient
            await self.page.keyboard.press("Control+a")
            await self.humaniser.pause(100, 250)
            await self.page.keyboard.press("Backspace")
            await self.humaniser.pause(100, 250)
            await self.page.keyboard.type(to_address, delay=80)
            await self.humaniser.pause(300, 700)
            ...
            # Send — use Gmail keyboard shortcut Ctrl+Enter (most reliable).
            # Button locators fail because Gmail keeps hidden compose windows in the DOM.
            try:
                ...
                self.log.warning("Send shortcut failed, trying button...")
                # Fallback: try visible button click
                ...
                if await send_btn.is_visible(timeout=3000):
                    ...
                    self.log.info(f"Email sent to {to_address}")
                    EmailSendActivity._save_cooldown(to_address)
                    return True
